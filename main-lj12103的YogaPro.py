...
async def startup_event():
    ...
    if not success:
        ...
        if create_demo_nodes:
            ...
            try:
                ...
                for i in range(demo_node_count):
                    ...
                    if node_success:
                        created_nodes += 1
                        logger.info(f"成功创建演示节点: {node_id}")
                    else:
                        logger.warning(f"创建演示节点失败: {node_id}")
        ...
        # 获取Ray集群节点信息（这些节点已在initialize_ray中自动创建）
        ray_cluster_nodes = await cluster.get_ray_cluster_nodes_info()
        if ray_cluster_nodes:
            logger.info(f"发现并集成了 {len(ray_cluster_nodes)} 个Ray集群节点到传输系统")
            for node_id, node_info in ray_cluster_nodes.items():
                logger.info(f"Ray集群节点: {node_id} -> {node_info['address']}:{node_info['port']}")
        
        # 启动演示传输调度器（如果有任何节点）
        total_nodes = len(cluster.nodes)
        if total_nodes > 0:
            asyncio.create_task(scheduler.start_demo_transfers())
            logger.info(f"演示传输调度器已启动，管理 {total_nodes} 个节点")
        else:
            logger.warning("没有可用节点，跳过启动演示传输调度器")
...
@app.get("/api/ray-nodes/refresh")
async def refresh_ray_nodes():
    """刷新Ray集群节点"""
    try:
        ray_cluster_nodes = await cluster.get_ray_cluster_nodes_info()
        ...
        return {
            "success": True,
            "message": f"成功刷新Ray集群节点信息",
            "ray_nodes_count": len(ray_cluster_nodes),
            "ray_nodes": ray_cluster_nodes
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"刷新Ray集群节点失败: {str(e)}")

@app.get("/api/ray-nodes")
async def get_ray_nodes():
    """获取Ray集群节点列表"""
    try:
        ray_cluster_nodes = await cluster.get_ray_cluster_nodes_info()
        ...
        return {
            "success": True,
            "ray_nodes": ray_cluster_nodes,
            "total_count": len(ray_cluster_nodes)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取Ray集群节点失败: {str(e)}")
